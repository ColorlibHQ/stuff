...
class Stuff_Helper {
	...
	public static function get_layout( $option = '' ) {
		$layout = empty( $option ) ? get_theme_mod( 'stuff_layout', false ) : get_theme_mod( $option, false );

		if ( ! $layout ) {
			$layout = Stuff_Helper::get_blog_default();
		}
     ...
     * @return array|mixed|object|string
     */
    public static function get_page_layout( $option = '' ) {
        $layout = empty( $option ) ? get_theme_mod( 'stuff_page_layout', false ) : get_theme_mod( $option, false );
	...
	public static function posted_on( $element = 'default' ) {
		...
		switch ( $element ) {
			case 'author':
				$html = '<span class="byline">' . esc_html_e( 'by ', 'stuff' );
				...
				break;
			case 'category':
				$html = '<div class="cat-links">' . esc_html__( 'Categories: ', 'stuff' );
				...
				break;
			case 'comments':
				echo ' <span class="comments-link"><a title="' . esc_attr__( 'Comment on Post', 'stuff' ) . '" href="' . esc_url( get_the_permalink( get_the_ID() ) ) . '#comments">' . esc_html( $comments->approved ) . '</a></span>';
				break;
			case 'tags':
